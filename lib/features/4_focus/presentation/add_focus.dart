...
class AddFocus extends StatefulWidget {
  const AddFocus({super.key});

  @override
  _AddFocusState createState() => _AddFocusState();
}

class _AddFocusState extends State<AddFocus> {
  ...
  void _saveData() async {
    if (_formKey.currentState!.validate() && selectedSection != null) {
      ...
      await FocusTimerHelper.addFocusTimer(newTimer);

      if (!mounted) return;
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text("Data saved to database!")));
  ...
  void _discardData() async {
    ...
    if (confirm == true) {
      Navigator.pop(
        context,
        false,
      ); // Kembali ke halaman sebelumnya tanpa menyimpan
    }
  }
